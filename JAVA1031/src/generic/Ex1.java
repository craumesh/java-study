...
//		noc.data = "홍길동";
		...
		// 잘못된 타입 변환(다운캐스팅)으로 인해 오류 발생 가능
		//String name = (String)o;
		String name = "";
		if(String.valueOf(o) instanceof String) {
		...
		gc.setMember(1);
//		gc.setMember("홍길동"); // 지정한 정수 타입 외에 다른 모든 형태들은 에러 발생
...
//		gc3.setMember(1);
		...
		//	제네릭 타입 자체를 생략할경우 경고메세지가 표시되므로
		//	제네릭 사용 추천
		GenericClass<Object> gc6 = new GenericClass<Object>(); // 경고메세지 미표시
 ...
 * 	=> 가상의 자료형은 보통 1글자 영문 대문자 사용(주로 E또는 T사용)
 * - 가상의 자료형은 클래스 정의 시점에서 정확한 자료형을 명시하지 않지만
 * 	클래스 정의 시점에서 데이터 타입 대신 사용 가능함
...
class GenericClass<T> {
	...
	T member; // 멤버변수 member 의 데이터타입이 T 로 지정된(실제 데이터타입 X)
