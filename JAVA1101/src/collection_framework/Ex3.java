...
//		list.add(3.14); // 제네릭 타입 고정으로 인해 정수 외에 추가 불가
		...
		// => 단 제네릭 타입을 Integer 로 지정 했으므로 리턴타입 Integer
		System.out.println("1번 인덱스에 저장된 요소 : " + list.get(1));		
		// 배열과 마찬가지로 존재하지 않는 인덱스 지정할 경우 인덱스 에러 발생
		...
		// => 제네릭 타입에 따라 리턴타입 변함
		// => 요소가 정수 타입일 경우 정수 데이터를 지정하면
		// 		정수 데이터가 아닌 인덱스로 취급되므로
		//		정수형 요소를 지정해야 하는 경우 Object 타입으로 변환 필요
//		System.out.println("정수 2를 삭제 : " + list.remove((Object)2));
//		System.out.println("list 객체에 저장된 모든 요소 : " + list);
//		System.out.println("list 인덱스 2번 데이터를 삭제 : " + list.remove(2));
//		System.out.println("list 객체에 저장된 모든 요소 : " + list);
		
		// 보통, 요소를 삭제해야할 경우 indexOf()로 탐색된 인덱스 요소 삭제
		list.remove(list.indexOf(2));
		System.out.println("list.remove(list.indexOf(2)) 결과 : " + list);
		
		// add(int index, Object o) : 특정 인덱스에 해당 요소(o) 를 추가
		// => 기존 데이터는 뒤로 밀려남
		list.add(1, 5); // 1번 인덱스에 5 추가(기존 1번 인덱스 데이터로부터 뒤로 밀림)
		System.out.println("list 객체에 저장된 모든 요소 : " + list);
		...
		List subList = list.subList(1, 3); // 1번 인덱스 부터 2번까지 추출
...
//		List<Object> nameList = Arrays.asList(names);
		
		// asList() 메서드 파라미터로 가변인자가 사용되므로
		// 데이터를 직접 여러개 전달 가능하며, 배열로 자동으로 변환되어 전달됨
		List<Object> nameList = Arrays.asList("홍길동", "이순신", "강감찬");
