...
public class Ex1 {
	...
	public static void main(String[] args) {
		 ...
		 * 	ex) 메신저 내에서 파일 전송과 동시에 메세지를 송수신 하는 것
		 * - 하나의 새로운 프로세스를 실행하는 것보다
		 * 	하나의 새로운 쓰레드를 생성하는 것이 더 적은 비용이 듦
		 */
		
		NoThread nt1 = new NoThread("★A 작업★", 10000000);
		NoThread nt2 = new NoThread("●B 작업●", 10000000);
		NoThread nt3 = new NoThread("■C 작업■", 10000000);
		
		nt1.run(); // A 작업이 실행이 끝나면
		nt2.run(); // B 작업 실행되고, B 작업이 실행이 끝나면
		nt3.run(); // C 작업 실행된다
