...
public class Ex1 {
	...
	public static void main(String[] args) {
		 ...
		 * - 인스턴스 생성 전, 클래스가 메모리에 로딩될 때 함께 로딩 됨
		 * 	=> 즉, 클래스 당 하나만 생성됨(클래스 영역 = Method Area 에 생성됨)
		 * 	=> 인스턴스 생성 없이도 사용 가능
		 * - 클래스명.멤버변수명 형태로 접근 
		 */
		
		//System.out.println("NormalMember : " + NormalMember);
		// => static 이 정의되어 잇지 않은 인스턴스 멤버 변수 a에 접근하려고 하면 에러 발생
		
		NormalMember n1 = new NormalMember();
		NormalMember n2 = new NormalMember();
		// 두 개의 인스턴스가 생성되며, 인스턴스 멤버 변수도 각각 두 개씩 생성됨
		...
		// 인스턴스 멤버변수인 a와 b는 인스턴스 마다 생성되므로
...
class NormalMember {
	int a = 10; // 인스턴스 변수(= 멤버 변수 = 필드 )
	int b = 20;	// 인스턴스 변수
}

class StaticMember {
	static int a = 10; // 클래스 멤버 변수(= static 멤버 변수 = 정적 멤버 변수)
	int b = 20; // 인스턴스 멤버 변수
}
