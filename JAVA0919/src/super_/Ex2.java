...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 * 				컴파일 에러 발생, 이 때 슈퍼클래스의 파라미터 생성자를
		 * 				명시적으로 호출하기 위해 사용
		 ...
		 * 				코드 중복을 제거하기 위해 사용
		 * 
		 * - 생성자 super()는 반드시 생성자의 첫 문장에서 실행되야 한다
...
class Student extends Person {
	...
	public Student() {
		...
		// => 그러므로 Person() 생성자 없이 파라미터 생성자만 있는 경우 오류
		// 1. 슈퍼클래스에 기본 생성자를 별도로 정의하거나
...
//		this.isStudyMode = isStudyMode;
		...
		this.isStudyMode = isStudyMode; // 서브클래스 생성자에서 isStudyMode 직접 초기화
	}
